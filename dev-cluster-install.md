...
На каждой из машин при установке нужно установить статический ip (ибо если вдруг будет рестарт машины то по dhcp она может получить адрес не тот,что зарегистрирован в кластере), удалить swap, отключить selinux и firewall.
...
ansible-playbook -i inventory/dev/hosts.yaml -b cluster.yml --ask-become-pass --skip-tags=multus
